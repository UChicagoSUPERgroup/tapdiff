...
export class RuleCreatorComponent implements OnInit {
  ...
  goToRulesHome() {
    if (this.userDataService.use_version) {
      this.router.navigate(["diff/" + this.userDataService.hashed_id + "/" + this.userDataService.task_id + "/" + this.userDataService.version + "/rules"]);
    } else {
