...
def _set_opt_to_autotap(opt):
    if isinstance(opt, m.SetParamOpt):
        opt = str(opt.value)
...
def _location_statement_clause_generator_set(dev_autotap, cap_autotap, par_autotap, comp, val, neg):
    """
    handle special case: it is a location sensor statement
    :param dev_autotap:
    :param cap_autotap:
    ...
    person_list = list(m.SetParamOpt.objects.filter(param_id=71).order_by('id'))
    # location_autotap_list = [_cap_name_to_autotap(loc.value) for loc in location_list]
    person_autotap_list = [_cap_name_to_autotap(per.value)+'_location' for per in person_list]
    # location_person_list = list(itertools.product(location_list, person_list))
    # location_person_autotap_list = [loc+'_'+per for loc, per in
    #                                 list(itertools.product(location_autotap_list, person_autotap_list))]
    location_autotap_list = [_set_opt_to_autotap(opt) for opt in location_list]
    # find the correct location
    location = location_list[location_autotap_list.index(val)]
    cap_par_autotap = cap_autotap + '_' + par_autotap
    person = person_list[person_autotap_list.index(cap_par_autotap)]
    # location, person = location_person_list[location_person_autotap_list.index(cap_par_autotap)]
...
def _update_clause_text(clause):
    ...
    # last-min hack for texts like "{who/=|}Anyone Exits Living Room" - 8/25/19 - VZ
            ...
            for par, par_val in zip(parameters, parameter_vals):
                if par['type'] == 'duration':
                    if par_val['comparator'] == '=':
                        ...
                        raise Exception('unexpected comparator %s' % par_val['comparator'])
        elif clause['capability']['id'] in [63, 70]:
            # location sensor
            for par, par_val in zip(parameters, parameter_vals):
                if par['id'] == 70 or par['id'] == 80:
                    if par_val['comparator'] == '=':
                        par_val['comparator'] = '!='
                    elif par_val['comparator'] == '!=':
                        par_val['comparator'] = '='
...
def autotap_simple_formula_to_clause(formula, flag=0):
    ...
    if dev_autotap == 'location_sensor_set':
        # special case 1: location sensor
        parameters, parameter_vals = _location_statement_clause_generator_set(
            dev_autotap, cap_autotap, par_autotap, comp, val, neg)
        capability = m.Capability.objects.get(id=70)
    elif dev_autotap == 'location_sensor':
...
def generate_all_device_templates():
    ...
    """
    template_dict = dict()
    irregular_cap_list = [9, 25, 26, 56, 35, 30, 31, 33, 32, 52, 51, 49, 50, 29, 63, 37, 70]
        ...
        for cap in cap_list:
            if cap.id not in irregular_cap_list:
                ...
                for person in person_list:
                    var_name = person.value + ' ' + 'location'
                    var_name = _var_name_to_autotap(var_name)
                    var_name = ''.join([ch for ch in var_name if ch.isalnum() or ch == '_'])
                    var_name = var_name.lower()
                    var_type = 'set, external'

                    value_list = [_set_opt_to_autotap(opt.value) for opt in location_list]
                    value_list = ', '.join(value_list)
                    value_list = ', [' + value_list + ']'
                    var_type += value_list
    ...
    if clause['capability']['id'] not in irregular_cap_list:
        ...
        if clause['capability']['id'] == 70:
            ...
            if location_val['comparator'] != '!=' and who_val['comparator'] != '!=':
                clause_statement = var_name + '=' + location_autotap_val
            else:
                clause_statement = var_name + '!=' + location_autotap_val
        elif clause['capability']['id'] == 63:
            # this is location sensor
            param_list = clause['parameters']
            param_val_list = clause['parameterVals']
...
def clause_to_autotap_tap(rule_clause):
    """
    ...
    """
    try:
        trigger_clause = rule_clause['ifClause'][0]
        trigger_statement = trigger_to_autotap_statement(trigger_clause, True)

        condition_list = rule_clause['ifClause'][1:]
        condition_statement_list = [trigger_to_autotap_statement(cond, False) for cond in condition_list]

        action_clause = rule_clause['thenClause'][0]  # only support one action
        action_statement = trigger_to_autotap_statement(action_clause, True)
...
def get_number_of_options(condition):
    parameters = list(m.Parameter.objects.filter(cap__id=condition['capability']['id']))
    if len(parameters) > 1:
        # should be location, need to distinguish set vs bin
        if condition['capability']['id'] == 70:  # ugly fix, set
            parameter = None
            for param in parameters:
                if param.name == 'location':
                    parameter = param
                    break
            opts = list(m.SetParamOpt.objects.filter(param=parameter))
            return len(opts)
        else:  # bin
            return 2
        ...
        if parameter.type == 'set':
            ...
            # TODO: should be based on number of critical values
            return 2
        else:
            return 2


def autotap_location_cond_post_process(condition_clause_list):
    new_condition_clause_list = []
    for cond in condition_clause_list:
        if cond['capability']['id'] == 63:  # location sensor
            new_cond = deepcopy(cond)
            param_who_id = [i for param, i in zip(new_cond['parameters'], range(len(new_cond['parameters']))) 
                            if param['name'] == 'who'][0]
            param_val_who = new_cond['parameterVals'][param_who_id]
            param_status_id = [i for param, i in zip(new_cond['parameters'], range(len(new_cond['parameters']))) 
                               if param['name'] == 'location'][0]
            who = param_val_who['value']
            new_cond['capability']['name'] = new_cond['capability']['name'] + ' ' + who
            new_cond['device']['name'] = new_cond['device']['name'] + ' ' + who
            new_cond['parameters'] = [new_cond['parameters'][param_status_id]]
            new_cond['parameterVals'] = [new_cond['parameterVals'][param_status_id]]
            
            new_condition_clause_list.append(new_cond)
        else:
            new_condition_clause_list.append(cond)
    return new_condition_clause_list


def autotap_diff_to_clause(event, condition, beh_tuple):
    event_clause = autotap_formula_to_clause(event, flag=1)
    condition_clause_list = [autotap_formula_to_clause(cond, flag=0) for cond in condition]
    # need to handle location
    new_condition_clause_list = autotap_location_cond_post_process(condition_clause_list)

    beh1 = beh_tuple[0]
    beh2 = beh_tuple[1]
    behavior1_clause = [autotap_formula_to_clause(b, flag=2) for b in beh1]
    behavior2_clause = [autotap_formula_to_clause(b, flag=2) for b in beh2]
    return {'event': event_clause, 'condition': new_condition_clause_list,
                            'behavior1': behavior1_clause, 'behavior2': behavior2_clause}


def autotap_qdiff_to_clause(event, condition, beh_list, all_vers):
    event_clause = autotap_formula_to_clause(event, flag=1)
    condition_clause_list = [autotap_formula_to_clause(cond, flag=0) for cond in condition]
    new_condition_clause_list = autotap_location_cond_post_process(condition_clause_list)

    behavior_clause_list = [[autotap_formula_to_clause(b, flag=2) for b in beh] for beh in beh_list]
    return {'event': event_clause, 'condition': new_condition_clause_list, 
            'behavior_list': behavior_clause_list}
