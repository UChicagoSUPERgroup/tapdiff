    ...
    def __new__(cls, name, parent, dct):
        ...
        for key, value in dct.items():
            if key.startswith('enable_') and callable(value):
                ...
                ap_dict[key[3:]] = value
            # why did I write not bool here before?
            elif isinstance(value, int) or \
                    isinstance(value, float) or \
                    (isinstance(value, str) and not key.startswith('_')) or \
...
class Channel(metaclass=MetaChannel):
    ...
    def saveToStateVector(self):
        ...
        state_list = [s if not isinstance(s, bool) else int(s) for s in state_list]
        return state_list

    # def __repr__(self):
    #     return str(self.__class__.__dict__.items())
