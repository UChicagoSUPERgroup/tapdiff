...
class IoTSystem(metaclass=MetaIoTSystem):
    ...
    def _tapConditionSatisfied(self, tap):
        for cond in tap.condition:
            if cond.startswith('!'):
                cond2 = cond[1:]
                if self._conditionSatisfied(cond2):
                    return False
            else:
    ...
    def _applyAction(self, action):
        ...
        for tap_name, tap in self.tap_dict.items():
            if tap.trigger and tap.trigger == action and \
                    self._tapConditionSatisfied(tap) and self._actionEnabled(action):
    ...
    def _actionEnabled(self, action):
        channel_name, action_name = action.split('.')
        if channel_name not in self.channel_dict:
            raise Exception('Unfound channel %s' % channel_name)
        if action_name not in self.channel_dict[channel_name].action_dict:
            raise Exception('Unfound action %s in channel %s' % (action_name, channel_name))
        return self.channel_dict[channel_name].enable_dict[action_name](self.channel_dict[channel_name])
    ...
    def getCondensedApList(self, field, template_dict):
        result = list()
        self._restoreFromStateVector(field)
        for ch_name, channel in sorted(self.channel_dict.items()):
            for state_name, state in sorted(channel.state_dict.items()):
                if template_dict[ch_name][state_name].startswith('bool'):
                    state = 'true' if state else 'false'
                else:
                    state = str(state)
                result.append('%s.%s=%s' % (ch_name, state_name, state))
    ...
    def getPureStateList(self, field=None):
        if field is not None:
            self._restoreFromStateVector(field)
        val_list = []
        for ch_name, channel in sorted(self.channel_dict.items()):
            for cap_name, val in sorted(channel.state_dict.items()):
                val_list.append(val)
        val_list = [val if not isinstance(val, bool) else int(val) for val in val_list]
