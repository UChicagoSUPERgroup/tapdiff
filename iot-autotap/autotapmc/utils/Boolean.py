...
def calculateBooleanZ3(formula, var_dict):
    """
    given some of the boolean variable value, simplify the formula (!, &, |, brackets are supported)
    :param formula:
    :param var_dict:
    :return:
    """
    ...
    for token in token_list:
        if token == '!':
            stack[-1] = z3.Not(stack[-1])
        elif token == '&':
            stack[-2] = z3.And(stack[-1], stack[-2])
            stack.pop()
        elif token == '|':
            stack[-2] = z3.Or(stack[-1], stack[-2])
            stack.pop()
        elif token in var_dict:
            stack.append(var_dict[token])
        elif token == '0':
...
def xorGenerator(a_str, b_str):
    return '((!(%s) & (%s)) | ((%s) & !(%s)))' % (a_str, b_str, a_str, b_str)
