...
class BuchiEdge(object):
    ...
    def __repr__(self):
        return '%s to %s: %s || %s' % (self.src, self.dst, self.ap, self.description)
...
class BaseBuchi(object):
    ...
    def __repr__(self):
        return "\nBuchi:\n    aps:\n\t%s\n    init state: %s\n    states:\n\t%s\n    edges:\n\t%s\n" % \
            (',\n\t'.join(str(self.ap_list).split(', ')), str(self.init_state), 
            ',\n\t'.join(str(self.state_dict).split(', ')), ',\n\t'.join(str(self.edge_list).split(', ')))
