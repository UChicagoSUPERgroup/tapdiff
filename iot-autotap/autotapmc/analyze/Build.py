...
def _getValueDict(formula, with_timing_ops=True):
    value_dict = dict()
    if with_timing_ops:
...
def generateCriticalValue(ltl, tap_list, with_timing_ops=True):
    def isfloat(s):
        try:
            float(s)
            return True
    ...
    value_dict = _getValueDict(ltl, with_timing_ops)
    ...
    for tap in tap_list:
        for cond in tap.condition:
            value_dict = _getValueDict(cond, False)
            for key, val_l in value_dict.items():
                if key in crit_value_dict:
                    crit_value_dict[key] = list(set(crit_value_dict[key]+val_l))
                else:
    ...
    for tap in tap_list:
        ...
        value_dict = _getValueDict(trigger, False)
        for key, val_l in value_dict.items():
        ...
        value_dict = _getValueDict(tap.action, False)
        for key, val_l in value_dict.items():
            if all([isfloat(v) for v in val_l]):
                for val in val_l:
...
def getChannelList(ltl, tap_list, only_relevant=False):
    channel_list = list()
    re_splitter = r'(\s+|\(|\)|\&|\||!|<|>|=|F|G|U|W|\#|\*|X|@)'
    ops = {
        '!': Operator('!', 1, 4, 1),
    ...
    if only_relevant:
        ...
        for trigger_cap_l, action_cap_l, tap_index in tap_cap_list:
            for cap in trigger_cap_l + action_cap_l:
                if cap not in cap_list:
                    cap_list.append(cap)

        channel_list = [cap_name.split('.')[0] for cap_name in cap_list]
        new_tap_list = tap_list

    return list(set(channel_list)), list(set(cap_list)), new_tap_list


def getChannelListSimple(ltl, tap_list):
    channel_list = list()
    re_splitter = r'(\s+|\(|\)|\&|\||!|<|>|=|F|G|U|W|\#|\*|X|@)'
    ops = {
        '!': Operator('!', 1, 4, 1),
    ...
    for tap in tap_list:
        ...
        tap_channel_list.append((trigger_channel_l, action_channel_l, tap_list.index(tap)))
        cap_list = cap_list + trigger_cap_l + action_cap_l

    channel_list = [cap_name.split('.')[0] for cap_name in cap_list]

    return list(set(channel_list)), list(set(cap_list))
...
def tapFormat(tap_list, critical_value_dict, template_dict=default_template_dict):
    def isfloat(s):
        try:
            float(s)
            return True
    ...
    for tap in tap_list:
        ...
        if not re.match(r'^tick\[(?P<time_exp>[()\w<>=#*.]+)\]$', tap.trigger):
            if tap.trigger.startswith('!'):
                tap_trigger = tap.trigger[1:]
                neg = True
            else:
                tap_trigger = tap.trigger
                neg = False

            trigger_post_exp = parse(tap_trigger, ops, re_splitter)
            if trigger_post_exp[-1] == '<':
                ...
                if not isfloat(trigger_post_exp[-2]):
                    if not neg:
                        ...
                        dev, cap = trigger_post_exp[-3].split('.')
                        if template_dict[dev][cap].startswith('bool'):
                            val = 'true' if trigger_post_exp[-2] == 'false' else 'false'
                            new_trigger.append(trigger_post_exp[-3] + 'Set' + val.capitalize())
                        else:
                            opt_list = template_dict[dev][cap].split('[')[1].split(']')[0]
                            opt_list = opt_list.split(', ')
                            val_list = [opt for opt in opt_list if opt != trigger_post_exp[-2]]
                            for val in val_list:
                                new_trigger.append(trigger_post_exp[-3] + 'Set' + val.capitalize())
                else:
                    if not neg:
        ...
        for cond in tap.condition:
            if '#' in cond or '*' in cond:
                ...
                if cond_post_exp[-1] == '!':
                    neg = True
                    cond_post_exp = cond_post_exp[:-1]
                else:
                    neg = False
                if cond_post_exp[-1] == '=':
                    if (cond_post_exp[-2] == 'true' and not neg) or (cond_post_exp[-2] == 'false' and neg):
                        condition.append(cond_post_exp[-3]+'IsTrue')
                    elif (cond_post_exp[-2] == 'false' and not neg) or (cond_post_exp[-2] == 'true' and neg):
                        condition.append('!'+cond_post_exp[-3]+'IsTrue')
                        ...
                        if not neg:
                            ...
                            raise Exception('currently do not support not equal for range variables')
                    else:
                        # should be 'set'
                        if not neg:
...
def changeBooleanToInt(field):
    return [int(v) if isinstance(v, bool) else v for v in field]

def getSetOptions(template_dict):
    set_value_dict = dict()
    for dev_name, cap_dict in template_dict.items():
        for var_name, var_type in template_dict[dev_name].items():
            if 'set' in var_type:
                # This converts "[R, G, B]" to "['R', 'G', 'B']"
                # The original regex made R&B become two items for amazon_echo.genre_genre
                var_type = var_type.split('[')[1]
                var_type = var_type.split(']')[0]
                value_list = var_type.split(', ')
                
                cap_name = '%s.%s' % (dev_name, var_name)
                set_value_dict[cap_name] = value_list
